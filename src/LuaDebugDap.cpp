...
namespace LuaDebug
{
		...
		const std::vector<BreakpointId> &hitBreakpoints)
	{
		...
		if( !hitBreakpoints.empty() )
		{
			...
			for( BreakpointId id : hitBreakpoints )
				if( id != 0 ) event["body"]["hitBreakpointIds"].append(id);
		}
		return event;
	}
		...
		int lineNumber)
	{
		...
		if( !sourceName.empty() ) event["body"]["source"]["name"] = sourceName;
		if( !source.empty() ) event["body"]["source"]["path"] = source;
		if( lineNumber >= 0 ) event["body"]["line"] = lineNumber;
		return event;
	}
	...
	{
		return CreateDapEvent("terminated");
	}

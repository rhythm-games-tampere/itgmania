...
namespace LuaDebug
{
	...
	public:
		...
		static Breakpoint Line(BreakpointId id, const std::string &fileName, int lineNumber);

		bool IsHit(lua_State *L, lua_Debug &debug);
		bool EvaluateConditions(DebuggeeState &stackFrame, std::string &evaluationError);
		std::string EvaluateLogMessage(DebuggeeState &stackFrame, std::string &evaluationError);
	...
	private:
		...
		int m_lineNumber = -1;
		/* If nonnegative, the breakpoint only triggers when the call stack depth is at or below the limit. */
		...
		/* If positive, the breakpoint only triggers after hit count reaches hit condition. */

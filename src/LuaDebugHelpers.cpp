...
namespace LuaDebug
{
	std::string LuaQuote(const std::string &s)
	{
		...
		for( unsigned char c : s )
		{
			...
			else if( 0x20 <= c && c <= 0x7e ) result.push_back(c);
			// Numerical escapes
			else
			{
				result.push_back('\\');
	...
	std::string LuaValueToString(lua_State *L, int index, bool quote)
	{
		...
		switch( type )
		{
			...
			case LUA_TBOOLEAN:
				...
				break;
			case LUA_TLIGHTUSERDATA: value = "<lightuserdata>"; break;
			...
			case LUA_TSTRING:
			{
				...
				value = std::string(buffer, length);
				if( quote ) value = LuaQuote(value);
				break;
			}

			case LUA_TTABLE: value = "{...}"; break;
			case LUA_TFUNCTION: value = "<function>"; break;
			case LUA_TUSERDATA: value = "<userdata>"; break;
			case LUA_TTHREAD: value = "<thread>"; break;
		}
		return value;
	}
	...
	int LuaEvaluate(const std::string &expression, DebuggeeState &frameOrThread)
	{
		...
		if( loadstringResult != 0 ) return -1;
		...
		if( frame )
		{
			...
			for( DebuggeeState *scope : scopes )
			{
				...
				{
					if( !var->IsVariable() || var->IsTemporaryVariable() ) continue;
			...
			// [..., expression, environment, env metatable, "__index", current function]
			lua_getfenv(L, -1);
			// [..., expression, environment, env metatable, "__index", current function, function environment]
			lua_remove(L, -2);
			// [..., expression, environment, env metatable, "__index", function environment]
			lua_rawset(L, -3);
			// [..., expression, environment, env metatable]
	...
	std::string ResolvePath(const std::string &luaPath)
	{
		...
		// Drop leading '@'
		if( pathView.substr(0, 1) != "@" ) return "";
		pathView = pathView.substr(1);

		int error = 0;
		std::unique_ptr<RageFileBasic> sourceFile(FILEMAN->Open(std::string(pathView), RageFile::READ, error));
		if( sourceFile ) return sourceFile->GetDisplayPath();
	...
	std::vector<std::string> UnresolvePath(const std::string &absolutePath)
	{
		...
		{
			RString root = mount.Root;
			if( root.Right(1) != "/" ) root.append("/");
			if( root == "/" ) continue;
			if( absPath.Left(root.size()).CompareNoCase(root) ) continue;

			RString path = absPath.Right( absPath.size() - root.size() );

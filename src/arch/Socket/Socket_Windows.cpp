...
public:
	...
	static WindowsTcpServerSocket* Listen(std::string_view host, std::string_view port)
	{
		...
		if( getaddrinfo(hostString.c_str(), portString.c_str(), &hints, &addressInfo) != 0 ) goto error;
...
public:
	...
	int Receive(char *buffer, int length, std::string &clientAddress, int timeoutMilliseconds = -1)
	{
		...
		if( recvResult < 0 ) LogWsaError("recvfrom");
		if( recvResult < 0 ) return -1;
	...
	int Broadcast(const char *buffer, int length)
	{
		...
		if( sendResult < 0 ) LogWsaError("sendto");
		return sendResult;
	}

	static WindowsUdpBroadcastSocket* Listen(std::string_view host, std::string_view port)
	{
		SOCKET listenSocket = INVALID_SOCKET;
		struct addrinfo *addressInfo = nullptr;
		const char *operation = "";
		...
		if( setsockopt(listenSocket, SOL_SOCKET, SO_BROADCAST, (char*)&yes, sizeof(yes)) != 0 ) goto error;
		if( setsockopt(listenSocket, SOL_SOCKET, SO_REUSEADDR, (char*)&yes, sizeof(yes)) != 0 ) goto error;
...
ServerSocket* ServerSocket::Listen(const char* address)
{
	...
	return WindowsTcpServerSocket::Listen(hostAndPort.first, hostAndPort.second);
}

BroadcastSocket* BroadcastSocket::Listen(const char* address)
{
	auto hostAndPort = SplitToHostAndPort(address);
	return WindowsUdpBroadcastSocket::Listen(hostAndPort.first, hostAndPort.second);
}

...
bool operator<(const ScorePlayer& l, const ScorePlayer& r) {
  return std::tie(l.machineAddress, l.playerNumber) <
         std::tie(r.machineAddress, r.playerNumber);
}
...
float getScore(const SyncStartScore& score) {
  ...
  return std::clamp(
      score.data.actualDancePoints / (float)score.data.possibleDancePoints,
      0.0f, 1.0f);
}
...
bool sortByLostScore(const SyncStartScore& l, const SyncStartScore& r) {
  // if a connection to a machine is dropped or something, don't keep its
  // position by lost score
  if (abs(l.data.currentPossibleDancePoints -
          r.data.currentPossibleDancePoints) >
      MAX_POSSIBLE_DANCE_POINTS_DIFFERENCE) {
    return sortByScore(l, r);
  }

  // negation so order is correct. unlike in score, less lost dance points is
  // better.
...
void SyncStartScoreKeeper::AddScore(
    const ScorePlayer& scorePlayer, const ScoreData& scoreData) {
...
std::vector<SyncStartScore> SyncStartScoreKeeper::GetScores(bool latestValues) {
  ...
  std::sort(
      ...
      latestValues ? sortByScore : sortByLostScore);
  return scores;
}

void SyncStartScoreKeeper::ResetScores() { this->scoreBuffer.clear(); }

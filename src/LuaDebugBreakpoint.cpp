...
namespace LuaDebug
{
	...
	Breakpoint Breakpoint::Function(BreakpointId id, const std::string &functionName)
	{
		Breakpoint bp;
		bp.m_id = id;
	...
	Breakpoint Breakpoint::Line(BreakpointId id, const std::string &fileName, int lineNumber)
	{
		Breakpoint bp;
		bp.m_id = id;
		bp.m_eventMask = LUA_MASKLINE;
	...
	{
		if( (m_eventMask & (1 << debug.event)) == 0) return false;
		if( m_thread != nullptr && m_thread != L ) return false;
		if( !m_functionName.empty() && (debug.name == nullptr || m_functionName != debug.name) ) return false;
		if( !m_fileName.empty() && (debug.source == nullptr || m_fileName != debug.source) ) return false;
		if( m_lineNumber >= 0 && m_lineNumber != debug.currentline ) return false;
		lua_Debug tmp;
		if( m_maxStackDepth >= 0 && lua_getstack(L, m_maxStackDepth, &tmp) ) return false;
		return true;
	}

	bool Breakpoint::EvaluateConditions(DebuggeeState &stackFrame, std::string &evaluationError)
	{
		...
		if( m_hitCondition > 0 && ++m_hitCount < m_hitCondition ) return false;

		m_hitCount = 0;
		return true;
	}

	std::string Breakpoint::EvaluateLogMessage(DebuggeeState &stackFrame, std::string &evaluationError)
	{
		// Evaluates any Lua expressions inside { } in the log message.

		if( m_logMessage.empty() ) return m_logMessage;
		lua_State *L = stackFrame.GetLua();
		...
		for( size_t i = 0; i < m_logMessage.size(); i++ )
		{
			if( m_logMessage[i] == '{' )
			{
				...
				else
				{
					...
					{
						if( m_logMessage[i] != '}' ) continue;
						// Found a closing }. Check if the substring from
						// expressionStart to i forms a syntactically valid Lua
						// expression.

						std::string expression = m_logMessage.substr(expressionStart, i - expressionStart);
						...
						{
							if( resultIndex > 0 ) result.append(", ");
			...
			else result.push_back(m_logMessage[i]);
		}

...
namespace LuaDebug
{
	...
	public:
		Impl():
			m_luaDebugMutex("LuaDebug"),
			m_debuggeeState(DebuggeeState::Root{})
		{
		...
		void SendRequest(Request &&request)
		{
			...
			if( !isLocked ) m_luaDebugMutex.Lock();
			if( m_isPaused )
			{
			...
			if( !isLocked ) m_luaDebugMutex.Unlock();
		}

		bool TryReceiveResponse(Response &response)
		{
			LockMutex lock(m_luaDebugMutex);
			if( m_toServerThread.empty() ) return false;
			response = m_toServerThread.front();
		...
		void SendResponse(Response &&response)
		{
			bool isLocked = m_luaDebugMutex.IsLockedByThisThread();
			if( !isLocked ) m_luaDebugMutex.Lock();
			if( m_client.initialized )
			{
				m_toServerThread.push(response);
				m_luaDebugMutex.Broadcast();
			}
			if( !isLocked ) m_luaDebugMutex.Unlock();
		}
		...
		{
			LockMutex lock(m_luaDebugMutex);
			if( m_toLuaThread.empty() ) return false;
	...
	private:
		...
		void ExecuteInitializeCommand(const Request &request)
		{
			...
			m_client.linesOffset = args["linesStartAt1"].isBool() && !args["linesStartAt1"].asBool() ? -1 : 0;
			m_client.columnsOffset = args["columnsStartAt1"].isBool() && !args["columnsStartAt1"].asBool() ? -1 : 0;
			m_client.supportsVariableType = args["supportsVariableType"].asBool();
			m_client.supportsInvalidatedEvent = args["supportsInvalidatedEvent"].asBool();
			m_client.initialized = true;
			...
			if( m_isPaused ) SendResponse(CreateStoppedEvent(m_currentThread, "entry", {}));
		}
		...
		void ExecuteSetBreakpointsCommand(const Request &request)
		{
			...
			for( int i = 0; (size_t)i < breakpoints.size(); i++ )
			{
				...
				if( lineNumber >= 0 ) lineNumber -= m_client.linesOffset;
		...
		{
			if( !m_isPaused ) return SendResponse(CreateNotStoppedResponse(request));
		...
		void ExecuteContinueCommand(const Request &request)
		{
			ContinueExecution();
			SendResponse(CreateDapResponse(request));
			if( !m_isPaused ) SendResponse(CreateContinuedEvent(0));
		}
		...
		void ExecuteEvaluateCommand(const Request &request)
		{
			...
			DebuggeeState &frameOrThread = frameId
				? m_debuggeeState.GetObject(frameId)
				: m_debuggeeState.GetThread(m_currentThread);
			if( !frameOrThread ) return SendResponse(CreateErrorResponse(request, "no such frame"));
			...
			for( int i = 0; i < resultCount; i++ )
			{
				...
				if( !resultType.empty() ) resultType.append(", ");
			...
			if( m_client.supportsVariableType ) response["body"]["type"] = resultType;
		...
		void ExecuteSetVariableCommand(const Request &request)
		{
			...
			if( !parent ) return SendResponse(CreateErrorResponse(request, "no such variable"));
			lua_State *L = parent.GetLua();

			DebuggeeState &var = parent.GetVariable(varName);
			if( !var ) return SendResponse(CreateErrorResponse(request, "no such variable"));
			...
			if( resultCount > 1 ) lua_pop(L, resultCount - 1);
			if( resultCount < 1 ) lua_pushnil(L);

			int luaType = lua_type(L, -1);
			std::string value = LuaValueToString(L, -1);
			std::string type = lua_typename(L, luaType);
			if( !var.PopValue() )
				return SendResponse(CreateErrorResponse(request, "error setting variable"));

			Response response = CreateDapResponse(request);
			response["body"]["value"] = value;
			if( m_client.supportsVariableType ) response["body"]["type"] = type;
			if( luaType == LUA_TTABLE ) response["body"]["variablesReference"] = var.GetId();
			SendResponse(std::move(response));
		}
		...
		void ExecuteThreadsCommand(const Request &request)
		{
			...
			{
				if( thread->GetType() != DebuggeeState::Type::Thread ) continue;
		...
		void ExecuteStackTraceCommand(const Request &request)
		{
			...
			for( size_t i = startFrame; i < thread.GetChildren().size() && framesJson.size() < (size_t)levels; i++ )
			{
				DebuggeeState &frame = *thread.GetChildren()[i];
				if( frame.GetType() != DebuggeeState::Type::StackFrame ) continue;
				lua_Debug debug;
				if( !lua_getstack(frame.GetLua(), frame.GetStackLevel(), &debug) ) continue;
		...
		void ExecuteVariablesCommand(const Request &request)
		{
			...
			for( DebuggeeState *var : parent.GetChildren() )
			{
				...
				if( skipNamed && var->GetType() != DebuggeeState::Type::IndexedVariable ) continue;
				if( skipIndexed && var->GetType() == DebuggeeState::Type::IndexedVariable ) continue;
				if( !var->PushValue() ) continue;
		...
		void HandleRequest(const Request &request)
		{
			try
			{
				...
				if( request["type"].asString() != "request" ) return;
				if( !request["seq"].isUInt64() ) return;

				std::string command = request["command"].asString();
				if( command == "attach" ) return ExecuteAttachCommand(request);
				if( command == "configurationDone" ) return ExecuteConfigurationDoneCommand(request);
				if( command == "continue" ) return ExecuteContinueCommand(request);
				if( command == "disconnect" ) return ExecuteDisconnectCommand(request);
				if( command == "evaluate" ) return ExecuteEvaluateCommand(request);
				if( command == "initialize" ) return ExecuteInitializeCommand(request);
				if( command == "next" ) return ExecuteNextCommand(request);
				if( command == "pause" ) return ExecutePauseCommand(request);
				if( command == "scopes" ) return ExecuteScopesCommand(request);
				if( command == "setBreakpoints" ) return ExecuteSetBreakpointsCommand(request);
				if( command == "setFunctionBreakpoints" ) return ExecuteSetFunctionBreakpointsCommand(request);
				if( command == "setVariable" ) return ExecuteSetVariableCommand(request);
				if( command == "stackTrace" ) return ExecuteStackTraceCommand(request);
				if( command == "stepIn" ) return ExecuteStepInCommand(request);
				if( command == "stepOut" ) return ExecuteStepOutCommand(request);
				if( command == "threads" ) return ExecuteThreadsCommand(request);
				if( command == "variables" ) return ExecuteVariablesCommand(request);
		...
		void LuaHook(lua_State *L, lua_Debug *ar)
		{
			...
			DebuggeeState &currentStackFrame = m_debuggeeState.GetThread(L).GetStackFrame(0);
			...
			for( size_t i = 0; i < m_breakpoints.size(); i++ )
			{
				...
				if( !bp.IsHit(L, *ar) ) continue;

				std::string evalError;
				if( !bp.EvaluateConditions(currentStackFrame, evalError) ) continue;
				if( !evalError.empty() )
				{
					m_isPaused = true;
					if( !evaluationErrors.empty() ) evaluationErrors.append("\n");
					evaluationErrors.append(evalError);
					if( bp.GetId() != 0 ) hitBreakpointIds.push_back(bp.GetId());
					continue;
				}
				...
				std::string logError;
				std::string logMessage = bp.EvaluateLogMessage(currentStackFrame, logError);
				if( !logError.empty() )
				{
					m_isPaused = true;
					if( !evaluationErrors.empty() ) evaluationErrors.append("\n");
				...
				if( !logMessage.empty() )
					SendResponse(CreateOutputEvent(logMessage, sourceName, source, lineNumber));
			...
			if( m_isPaused )
			{
				if( !evaluationErrors.empty() )
					...
					SendResponse(CreateStoppedEvent(
						m_currentThread,
						reason,
						hitBreakpointIds));
	...
	void Debugger::SendRequest(Request &&request) { m_impl->SendRequest(std::move(request)); }
	bool Debugger::TryReceiveResponse(Response &response) { return m_impl->TryReceiveResponse(response); }
}

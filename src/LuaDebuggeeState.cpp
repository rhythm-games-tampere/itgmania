...
namespace LuaDebug
{
	...
	public:
		...
		static void CreateThread(DebuggeeState &root, ThreadId threadId, lua_State *L)
		{
			auto thread = new DebuggeeState();
			thread->m_type = Type::Thread;
			thread->m_parent = &root,
			thread->m_lua = L,
		...
		static void CreateNamedVariable(DebuggeeState &parent, const std::string &name)
		{
		...
		static void CreateIndexedVariable(DebuggeeState &parent, const std::string &name, int index)
		{
		...
		static bool PushGlobalToStack(DebuggeeState &var)
		{
			...
			{
				if( !lua_getmetatable(L, LUA_GLOBALSINDEX) ) return false;
			}
			else FAIL_M("invalid type");
		...
		static bool PushReferenceToStack(DebuggeeState &var)
		{
			...
			if( var.m_parent->m_index == LUA_NOREF ) return false;
		...
		static bool PopUpvalueFromStack(DebuggeeState &var)
		{
			...
			lua_Debug debug;
			if( !lua_getstack(L, var.m_stackLevel, &debug) ) goto done;
			// Pushes the active function onto the stack.
			if( !lua_getinfo(L, "f", &debug) ) goto done;
			if( lua_isnil(L, -1) ) goto done;

			lua_insert(L, -2);
			if( lua_setupvalue(L, -2, var.m_index) == nullptr ) goto done;
			ok = true;
		...
		static void InitLocals(DebuggeeState &locals)
		{
			...
			if( locals.m_lua == nullptr ) return;
		...
		static void InitUpvalues(DebuggeeState &upvalues)
		{
			...
			if( upvalues.m_lua == nullptr ) return;
		...
		static void InitGlobals(DebuggeeState &globals)
		{
			...
			if( globals.m_lua == nullptr ) return;
		...
		static void InitThread(DebuggeeState &thread)
		{
			...
			if( thread.m_lua == nullptr ) return;
		...
		static void InitStackFrame(DebuggeeState &frame)
		{
			...
			if( frame.m_lua == nullptr ) return;

			lua_Debug debug;
			if( !lua_getstack(frame.m_lua, frame.m_stackLevel, &debug) ) return;
			if( !lua_getinfo(frame.m_lua, "u", &debug) ) return;

			CreateLocals(frame);
			if( debug.nups > 0 ) CreateUpvalues(frame);
		}
	...
	{
		DebuggeeState *o = this;
		while( o != nullptr && o->GetType() != Type::Thread ) o = o->m_parent;
	...
	{
		DebuggeeState *o = this;
		while( o != nullptr && o->GetType() != Type::StackFrame ) o = o->m_parent;
		return o == nullptr ? Impl::g_null : *o;
	}
	...
	DebuggeeState &DebuggeeState::GetObject(ObjectRef id)
	{
		...
		if( it == root.m_objects.end() ) return Impl::g_null;
		return it->second;
	}
	...
	DebuggeeState &DebuggeeState::GetThread(ThreadId threadId)
	{
		...
		{
			if( thread->GetType() == Type::Thread && thread->GetThreadId() == threadId )
				return *thread;
		}
		return Impl::g_null;
	}
	...
	DebuggeeState &DebuggeeState::GetStackFrame(int level)
	{
		...
		{
			if( frame->GetType() == Type::StackFrame && frame->GetStackLevel() == level )
				return *frame;
		}
		return Impl::g_null;
	}
	...
	DebuggeeState &DebuggeeState::GetGlobals()
	{
		...
		{
			if( child->GetType() == Type::GlobalsScope ) return *child;
		}
		return Impl::g_null;
	}
	...
	DebuggeeState &DebuggeeState::GetReferences()
	{
		for( DebuggeeState *child : GetThread().GetChildren() )
		{
			if( child->GetType() == Type::References ) return *child;
		}
		return Impl::g_null;
	}
	...
	DebuggeeState &DebuggeeState::GetUpvalues()
	{
		...
		{
			if( child->GetType() == Type::UpvaluesScope ) return *child;
		}
		return Impl::g_null;
	}
	...
	DebuggeeState &DebuggeeState::GetLocals()
	{
		for( DebuggeeState *child : GetStackFrame().GetChildren() )
		{
			if( child->GetType() == Type::LocalsScope ) return *child;
		}
		return Impl::g_null;
	}
	...
	DebuggeeState &DebuggeeState::GetVariable(const std::string &name)
	{
		...
		{
			if( child->IsVariable() && child->GetName() == name ) return *child;
		}
		return Impl::g_null;
	}
	...
	DebuggeeState &DebuggeeState::GetVariable(int index)
	{
		for( DebuggeeState *child : GetChildren() )
		{
			if( child->GetType() == Type::IndexedVariable && child->GetIndex() == index ) return *child;
		}
		return Impl::g_null;
	}
	...
	const std::vector<DebuggeeState*> &DebuggeeState::GetChildren()
	{
		if( !m_childrenInitialized )
		{
			switch( GetType() )
			{
				...
				case Type::Thread: Impl::InitThread(*this); break;
				case Type::StackFrame: Impl::InitStackFrame(*this); break;
				case Type::References: break;
				case Type::GlobalsScope: Impl::InitGlobals(*this); break;
				case Type::UpvaluesScope: Impl::InitUpvalues(*this); break;
				case Type::LocalsScope: Impl::InitLocals(*this); break;
				case Type::NamedVariable: Impl::InitVariable(*this); break;
				case Type::IndexedVariable: Impl::InitVariable(*this); break;
				case Type::MetaTableVariable: Impl::InitVariable(*this); break;
				default: FAIL_M("invalid type");
			}
	...
	{
		return
			m_type == Type::NamedVariable ||
			m_type == Type::IndexedVariable ||
	...
	{
		return
			m_type == Type::GlobalsScope ||
			m_type == Type::UpvaluesScope ||
	...
	bool DebuggeeState::PushValue()
	{
		...
		for( DebuggeeState *o = this; o != nullptr && o->IsVariable(); o = o->m_parent )
		...
		else
		{
			switch( m_parent->m_type )
			{
				...
				case Type::References:
					...
					break;
				default: FAIL_M("invalid scope type");
			}
		}
	...
	void DebuggeeState::Invalidate()
	{
		...
		for( size_t i = 0; i < m_children.size(); i++ )
		{
			...
			else
			{
				...
				if( keptItems != i ) std::swap(m_children[keptItems], m_children[i]);
				keptItems++;
			}
		}
	...
	{
		if( m_id != 0 ) GetRoot().m_objects.erase(m_id);

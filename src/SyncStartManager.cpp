...
#define ALL_ITEMS_LENGTH                      \
  (MISC_ITEMS_LENGTH + NUM_TapNoteScore + 1 + \
   NUM_HoldNoteScore)  // add 1 for white count
...
void SyncStartManager::broadcastScoreChange(
  ...
  std::stringstream msg = writeScoreMessage(
      pPlayerStageStats, false, w0Count, w1Count, w2Count, w3Count, w4Count,
      w5Count, missCount, currentDp, possibleDp, scoreStr);
  this->broadcast(SCORE, msg.str());
}

void SyncStartManager::broadcastFinalScore(
    const PlayerStageStats& pPlayerStageStats, int w0Count, int w1Count,
    int w2Count, int w3Count, int w4Count, int w5Count, int missCount,
    int currentDp, int possibleDp, const std::string& scoreStr) {
  std::stringstream msg = writeScoreMessage(
...
void SyncStartManager::broadcastFinalCourseScore(
    const PlayerStageStats& pPlayerStageStats, int w0Count, int w1Count,
    int w2Count, int w3Count, int w4Count, int w5Count, int missCount,
    int currentDp, int possibleDp, const std::string& scoreStr) {
  std::stringstream msg = writeScoreMessage(
      pPlayerStageStats, true, w0Count, w1Count, w2Count, w3Count, w4Count,
      w5Count, missCount, currentDp, possibleDp, scoreStr);
  this->broadcast(FINAL_COURSE_SCORE, msg.str());
}

std::stringstream SyncStartManager::writeScoreMessage(
    const PlayerStageStats& pPlayerStageStats, bool isCourseScore, int w0Count,
    int w1Count, int w2Count, int w3Count, int w4Count, int w5Count,
    int missCount, int currentDp, int possibleDp,
    const std::string& scoreStr) const {
  std::stringstream msg;

  std::string playerName =
      PROFILEMAN->GetPlayerName(pPlayerStageStats.m_player_number);
  ...
  // Tap note score values, get judgment counts from theme side to make rescored
  // judgments work
  ...
  int possibleHolds =
      (int)pPlayerStageStats.m_radarPossible[RadarCategory_Holds];
  int possibleRolls =
      (int)pPlayerStageStats.m_radarPossible[RadarCategory_Rolls];
...
class LunaSyncStartManager : public Luna<SyncStartManager> {
 public:
  static void PushScores(
      T* p, lua_State* L, const std::vector<SyncStartScore>& scores) {
    lua_newtable(L);
  ...
  static int BroadcastScoreChange(T* p, lua_State* L) {
    const PlayerStageStats* playerStageStats =
    ...
    p->broadcastScoreChange(
        *playerStageStats, w0Count, w1Count, w2Count, w3Count, w4Count, w5Count,
        missCount, currentDp, possibleDp, scoreStr);
    return 1;
  }
